...
# ─────────────────────────────────────────────────────────────────────────────
#  PAGE: MY SUBSCRIPTION  (billing management & cancellation)
# ─────────────────────────────────────────────────────────────────────────────
elif "My Subscription" in page:
    st.markdown('<div class="hero"><div class="hero-title">💳 MY SUBSCRIPTION</div><div class="hero-sub">Manage your plan — upgrade, downgrade, or cancel anytime</div></div>', unsafe_allow_html=True)
    st.markdown("---")

    # Determine current tier
    is_speed = st.session_state.get("speed_tier_unlocked", False)
    is_pro = st.session_state.get("pro_tier_unlocked", False)
    customer_id = st.session_state.get("stripe_customer_id")
    subscription_id = st.session_state.get("stripe_subscription_id")
    stripe_live = billing is not None and billing.stripe_configured()
    ...
    if is_speed:
        ...
        plan_color = "#F59E0B"
        plan_includes = [
            "Unlimited AI swing diagnoses",
            "Two-angle cross-referenced analysis",
            "Full AI coach with regression tracking",
            "Slow-mo & frame-by-frame review",
            "Equipment & club fitting recommendations",
            "Pro lesson integration",
            "📊 Benchmark comparisons",
            "⚡ 8-week swing-speed program",
            "⚡ Overspeed/overload protocol",
            "⚡ 16 speed-specific exercises",
            "⚡ AI speed coach + driver speed tracker",
        ]
    elif is_pro:
        plan_name = "PRO"
        ...
        plan_includes = [
            ...
            "Pro lesson integration",
            "📊 Benchmark comparisons",
            "Unlimited swing logbook & history",
        ]
    else:
        plan_name = "FREE"
        plan_price = "$0 forever"
        plan_color = "#6B8BAF"
        plan_includes = [
            "1 baseline swing assessment",
    ...
    st.markdown(f"""
    <div class="card" style="border:1px solid {plan_color};padding:1.6rem 1.8rem;">
      <div style="display:flex;justify-content:space-between;align-items:baseline;flex-wrap:wrap;gap:.6rem;">
        <span style="font-family:'Bebas Neue',sans-serif;color:{plan_color};font-size:1.6rem;letter-spacing:.06em;">CURRENT PLAN · {plan_name}</span>
        <span style="font-family:'Bebas Neue',sans-serif;font-size:1.4rem;color:#F0EBE0;">{plan_price}</span>
      </div>
    </div>
    """, unsafe_allow_html=True)
    st.markdown("")
    st.markdown("**What's included:**")
    for inc in plan_includes:
        st.markdown(f"- ✓ {inc}")

    # ── LIVE SUBSCRIPTION STATUS (if real Stripe sub) ──
    sub_status = None
    if stripe_live and subscription_id:
        sub_status = billing.get_subscription_status(subscription_id)
        if sub_status:
            st.markdown("---")
            st.markdown("### Billing Details")
            col1, col2, col3 = st.columns(3)
            status_color = {"active": "#4ADE80", "trialing": "#A855F7", "past_due": "#EF4444", "canceled": "#EF4444"}.get(sub_status["status"], "#F59E0B")
            with col1:
                st.markdown(f'<div class="metric-box"><div class="metric-val" style="color:{status_color};font-size:1.4rem;">{sub_status["status"].upper()}</div><div class="metric-lbl">Status</div></div>', unsafe_allow_html=True)
            with col2:
                if sub_status.get("current_period_end"):
                    period_end = datetime.datetime.fromtimestamp(sub_status["current_period_end"]).strftime("%b %d, %Y")
                else:
                    period_end = "—"
                end_label = "Access Until" if sub_status["cancel_at_period_end"] else "Next Billed"
                st.markdown(f'<div class="metric-box"><div class="metric-val" style="font-size:1.1rem;line-height:2.4rem;">{period_end}</div><div class="metric-lbl">{end_label}</div></div>', unsafe_allow_html=True)
            with col3:
                cancel_status = "PENDING" if sub_status["cancel_at_period_end"] else "ACTIVE"
                cancel_color = "#F97316" if sub_status["cancel_at_period_end"] else "#4ADE80"
                st.markdown(f'<div class="metric-box"><div class="metric-val" style="color:{cancel_color};font-size:1.4rem;">{cancel_status}</div><div class="metric-lbl">Auto-Renewal</div></div>', unsafe_allow_html=True)

            if sub_status["cancel_at_period_end"]:
                st.markdown(f"""
                <div class="card card-warn">
                  <strong>⏳ Your subscription is set to cancel.</strong> You'll continue to have access
                  through <strong>{period_end}</strong>, then your plan reverts to Free. No further charges.
                </div>
                """, unsafe_allow_html=True)
                if st.button("↩️  Keep my subscription (undo cancellation)", use_container_width=True):
                    if billing.reactivate_subscription(subscription_id):
                        st.success("✓ Subscription reactivated. You'll continue to be billed normally.")
                        st.rerun()
                    else:
                        st.error("Couldn't reactivate. Try again or use 'Manage in Stripe Portal' below.")

    st.markdown("---")

    # ── CANCEL SECTION (only show if user has any paid plan) ──
    if is_speed or is_pro:
        st.markdown("### Cancel My Subscription")
        st.markdown("""
        <div class="card">
          <strong>How cancellation works:</strong>
          <ul style="margin:.5rem 0 0;line-height:1.7;">
            <li>You can cancel <strong>anytime</strong> — no questions asked, no calls required</li>
            <li>You keep <strong>full access until the end of your current billing period</strong></li>
            <li>Your plan reverts to Free automatically after that date</li>
            <li>Your swing history, baseline, and saved data <strong>are preserved</strong> on the Free plan</li>
            <li>You can re-subscribe anytime — your data is waiting for you</li>
          </ul>
        </div>
        """, unsafe_allow_html=True)
        st.markdown("")

        # PATH A: Real Stripe subscription — use Customer Portal
        if stripe_live and customer_id:
            st.markdown("**Two options to cancel:**")
            col_a, col_b = st.columns(2)
            with col_a:
                portal_url = billing.customer_portal_url(customer_id)
                if portal_url:
                    st.link_button("⚙️  Manage in Stripe Portal (recommended)", portal_url, use_container_width=True)
                    st.caption("Update payment method, change plan, or cancel — all in one place.")
            with col_b:
                if not (sub_status and sub_status.get("cancel_at_period_end")):
                    # Single-click cancel via API
                    confirm_cancel = st.checkbox("I want to cancel my subscription", key="confirm_cancel_paid")
                    if confirm_cancel:
                        if st.button("❌  Cancel Subscription", use_container_width=True, type="secondary"):
                            if subscription_id and billing.cancel_subscription_at_period_end(subscription_id):
                                st.success("✓ Subscription cancelled. You'll keep access until your billing period ends.")
                                st.rerun()
                            else:
                                st.error("Could not cancel. Please use the Stripe Portal option.")

        # PATH B: Demo mode (no real Stripe subscription) — local toggle
        else:
            st.markdown("**Demo mode — cancel by removing access:**")
            st.caption("In production with Stripe, this opens Stripe's Customer Portal. In demo mode, this just toggles your access off.")
            confirm = st.checkbox("Yes, I want to cancel my subscription", key="confirm_cancel_demo")
            if confirm:
                if st.button("❌  Confirm Cancellation", use_container_width=True):
                    if is_speed:
                        st.session_state.speed_tier_unlocked = False
                    if is_pro:
                        st.session_state.pro_tier_unlocked = False
                    # Clear stashed Stripe IDs
                    for k in ["stripe_customer_id", "stripe_subscription_id", "_speed_checkout_url"]:
                        st.session_state.pop(k, None)
                    st.success("✓ Subscription cancelled (demo mode). You've been moved to the Free plan. Your data is preserved.")
                    st.rerun()
            else:
                st.caption("Check the box above to enable the cancel button.")

        # Cross-sell / down-grade options
        if is_speed:
            st.markdown("---")
            st.markdown("### Or Just Downgrade")
            st.caption("Switch from Speed → Pro to save $3/month while keeping unlimited AI coaching.")
            if st.button("⬇️  Downgrade to Pro ($9.99/mo)", use_container_width=True):
                st.info("To switch tiers, cancel your current plan above, then subscribe to Pro from the Benchmarks page. (Auto-downgrade flow coming soon.)")

    # ── FREE TIER: SHOW UPGRADE OPTIONS ──
    else:
        st.markdown("### Upgrade to a Paid Plan")
        col_p, col_s = st.columns(2, gap="medium")
        with col_p:
            st.markdown("""
            <div class="card" style="text-align:center;height:100%;">
              <div style="font-family:'Bebas Neue',sans-serif;font-size:1.2rem;color:#F0EBE0;letter-spacing:.06em;">PRO</div>
              <div style="font-family:'Bebas Neue',sans-serif;font-size:2.6rem;color:#4ADE80;line-height:1;margin:.3rem 0;">$9.99<span style="font-size:.85rem;color:#6B8BAF;"> /mo</span></div>
              <div style="font-size:.78rem;color:#6B8BAF;">Unlimited AI + benchmarks + slow-mo + equipment fit</div>
            </div>
            """, unsafe_allow_html=True)
            if st.button("Unlock Pro — $9.99/mo", use_container_width=True, key="upgrade_pro_from_sub"):
                st.session_state.pro_tier_unlocked = True
                st.success("Pro tier unlocked! (Demo mode — Stripe checkout in production.)")
                st.rerun()

        with col_s:
            st.markdown("""
            <div class="card" style="text-align:center;height:100%;border:1px solid #F59E0B;">
              <div style="font-family:'Bebas Neue',sans-serif;font-size:1.2rem;color:#F59E0B;letter-spacing:.06em;">⚡ SPEED · BEST VALUE</div>
              <div style="font-family:'Bebas Neue',sans-serif;font-size:2.6rem;color:#4ADE80;line-height:1;margin:.3rem 0;">$12.99<span style="font-size:.85rem;color:#6B8BAF;"> /mo</span></div>
              <div style="font-size:.78rem;color:#6B8BAF;">Everything in Pro + 8-week speed program</div>
            </div>
            """, unsafe_allow_html=True)
            if stripe_live:
                if st.button("⚡ Continue to Checkout", use_container_width=True, type="primary", key="upgrade_speed_from_sub"):
                    url = billing.create_checkout_session()
                    if url:
                        st.session_state["_speed_checkout_url"] = url
                if st.session_state.get("_speed_checkout_url"):
                ...
                if st.button("⚡ Unlock Speed — $12.99/mo", use_container_width=True, type="primary", key="upgrade_speed_demo_from_sub"):
                    st.session_state.speed_tier_unlocked = True
                    st.success("Speed tier unlocked! (Demo mode.)")
                    st.rerun()
    ...
    # ── BILLING FAQ ──
    st.markdown("---")
    st.markdown("### Billing FAQs")
    with st.expander("When does my cancellation take effect?"):
        st.markdown("Immediately — but you keep access until the end of your current billing period. If you cancelled on the 5th and your plan renews on the 20th, you have full access until the 20th. No further charges after that.")
    with st.expander("Will I get a refund for the remainder of the month?"):
        st.markdown("No — but you keep your full access until the period you've paid for ends. This is standard for monthly subscription services.")
    with st.expander("What happens to my swing data if I cancel?"):
        st.markdown("Your baseline, swing library, session history, and pro lessons are **all preserved** on the Free plan. You just lose access to premium features. If you re-subscribe later, everything is exactly where you left it.")
    with st.expander("Can I switch tiers without cancelling?"):
        st.markdown("Yes — in the Stripe Customer Portal, you can switch between Pro and Speed instantly. Stripe pro-rates the difference automatically. (For demo mode in this app, cancel and re-subscribe.)")
    with st.expander("Can I pause instead of cancel?"):
        st.markdown("Not directly in the app yet, but if you contact support we can pause your subscription for up to 3 months. Your data stays exactly as you left it.")
    with st.expander("Are there any hidden fees?"):
        st.markdown("No. Your only charge is the monthly subscription. We don't charge for setup, cancellation, or anything else. Sales tax may apply based on your location (handled by Stripe).")
